...
exports.createVoucher = async (req, res) => {
  try {
    ...
    return res.status(201).json({ success: true, message: 'Voucher created successfully', data: newVoucher });
  } catch (error) {
...
exports.applyVoucher = async (req, res) => {
  try {
    ...
    if (!code || totalAmount === undefined) {
      return res.status(400).json({ success: false, message: 'Voucher code and total amount are required' });
    }
    ...
    if (totalAmount < voucher.minOrderValue) {
      return res.status(400).json({ success: false, message: `Minimum order value of ${voucher.minOrderValue} required` });
    }

    // Check per-user limit
    if (userId) {
      // Check usages in Orders
      const orderUsages = await Order.countDocuments({ userId: userId, voucherCode: voucher.code, status: { $ne: 'cancelled' } });
      // Check usages in Bookings
      const bookingUsages = await Booking.countDocuments({ customerId: userId, voucherCode: voucher.code, status: { $nin: ['cancelled', 'rejected'] } });
      
      const totalUserUsages = orderUsages + bookingUsages;
      
      if (totalUserUsages >= voucher.usageLimitPerUser) {
        return res.status(400).json({ success: false, message: 'You have reached the maximum usage limit for this voucher' });
      }
    } else {
       ...
       if (req.body.customerPhone) {
         const orderUsages = await Order.countDocuments({ customerPhone: req.body.customerPhone, voucherCode: voucher.code, status: { $ne: 'cancelled' } });
         const bookingUsages = await Booking.countDocuments({ customerPhone: req.body.customerPhone, voucherCode: voucher.code, status: { $nin: ['cancelled', 'rejected'] } });
