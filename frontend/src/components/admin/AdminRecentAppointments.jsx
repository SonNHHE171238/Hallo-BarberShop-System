          ...
          <tbody className="text-on-surface font-body-md text-[14px]">
            {isLoading ? (
              ...
              bookings.map(booking => {
                ...
                const serviceName = booking.serviceId?.name || (booking.services && booking.services.map(s => s.name).join(', ')) || 'Chưa chọn';
                ...
                return (
                  <tr key={booking._id} className="border-b border-outline-gold/10 hover:bg-surface-container-high/50 transition-colors">
                    ...
                    <td className="py-5 px-8 text-primary font-label-md">
                      {booking.timeSlot} <br/>
                      <span className="text-[10px] text-outline">{new Date(booking.bookingDate).toLocaleDateString('vi-VN')}</span>
                    </td>
