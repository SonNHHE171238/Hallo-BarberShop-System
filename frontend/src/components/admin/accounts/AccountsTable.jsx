...
export default function AccountsTable({
  ...
  const getRoleBadge = (role) => {
    switch (role) {
      case "admin":
        return (
          <span className="inline-flex items-center px-2.5 py-1 rounded text-xs font-label-md font-medium bg-primary/10 text-primary border border-primary/20">
            Quản trị viên
          </span>
        );
      case "barber":
        return (
          <span className="inline-flex items-center px-2.5 py-1 rounded text-xs font-label-md font-medium border border-outline-variant text-on-surface-variant">
            Thợ cắt tóc
          </span>
        );
      case "staff":
  ...
  if (error)
    return (
      <div className="text-error p-4 bg-surface-container rounded border border-error/30 text-center">
        Lỗi tải dữ liệu tài khoản. Vui lòng tải lại trang.
      </div>
    );
    ...
    <div className="bg-surface-container/50 border border-outline-variant rounded-xl overflow-hidden backdrop-blur-sm shadow-[0_4px_30px_rgba(0,0,0,0.5)] flex-1 flex flex-col min-h-0">
        ...
        <table className="w-full text-left border-collapse whitespace-nowrap min-w-[800px]">
          <thead className="sticky top-0 bg-surface-container-low z-10 shadow-sm">
            <tr className="border-b border-outline-variant bg-surface-container-low text-on-surface-variant font-label-md text-label-md uppercase tracking-wider text-[10px] sm:text-xs">
              ...
              <th className="hidden lg:table-cell px-3 py-2 sm:px-4 sm:py-3 font-semibold text-right">
                Thao tác
              </th>
            </tr>
          </thead>
          ...
          >
            {!isMounted || isLoading ? (
              <tr>
                <td
                  colSpan="6"
                  className="px-4 py-6 text-center text-on-surface-variant animate-pulse"
                >
                  Đang tải dữ liệu...
                </td>
              </tr>
              ...
              paginatedAccounts.map((account) => (
                <React.Fragment key={account._id || account.id}>
                  ...
                  >
                    ...
                    <td className="px-3 py-2 sm:px-4 sm:py-2.5 text-on-surface-variant max-w-[120px] sm:max-w-none truncate">
                      {account.email}
                    </td>
                    <td className="px-3 py-2 sm:px-4 sm:py-2.5 text-on-surface-variant">
                      {account.phone || "N/A"}
                    </td>
                    <td className="px-3 py-2 sm:px-4 sm:py-2.5">
                    ...
                    <td className="hidden lg:table-cell px-3 py-2 sm:px-4 sm:py-2.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {["admin", "staff", "barber"].includes(
                          account.role,
                        ) && (
                          <span className="text-outline text-[9px] sm:text-[10px] uppercase tracking-wider">
                            Không thể xóa ở đây
                          </span>
                        )}
                      </div>
                      ...
                      >
                        <div className="flex items-center justify-end gap-3">
                          ...
                          {account.role === "customer" ? (
                            ...
                            <span className="text-outline text-[10px] uppercase tracking-wider">
                              Không thể xóa ở đây
                            </span>
                          )}
                        </div>
      ...
      {accountToDelete && accountToDelete.role === "barber" && (
        <BarberUpcomingBookingsModal
          ...
          barber={{
            id: accountToDelete._id || accountToDelete.id,
