...
export default function StaffBookings() {
  ...
  useEffect(() => {
    if (qrCodeData && checkInModal.isOpen && checkInModal.booking) {
      const updatedBooking = appointments.find(
        (b) => b._id === checkInModal.booking._id,
      );
  ...
  const handleStatusUpdate = async (status) => {
    ...
    try {
      ...
      const res = await staffDashboardService.updateStatus(
        checkInModal.booking._id,
        payload,
      );
      if (res) {
        toast.success(
          status === "completed"
            ? "Đã thu tiền & hoàn thành"
    ...
    <div className="w-full">
      <div className="flex-1 overflow-y-auto p-12 space-y-8 pb-24 max-w-[1400px] mx-auto animate-fade-in">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <span className="px-3 py-1 rounded-full bg-outline-variant/20 border border-outline-variant text-[10px] font-bold text-gold-dim tracking-widest uppercase">
            LIVE DASHBOARD
          </span>
        </div>
        ...
        <section className="flex flex-col lg:flex-row justify-between items-center gap-6">
          <div className="flex flex-wrap gap-6 items-center">
            <div className="flex items-center gap-3">
              <label className="font-label-md text-xs text-outline uppercase tracking-wider whitespace-nowrap">
                Ngày làm việc:
              </label>
              <div className="relative flex items-center gap-2 bg-surface-container border border-outline-variant px-4 h-11 rounded-lg focus-within:border-primary focus-within:ring-1 focus-within:ring-primary transition-all">
                <span className="material-symbols-outlined text-gold-dim text-lg z-10">
                  event
                </span>
              ...
              <div className="flex flex-col h-full justify-between flex-1">
                <div className="space-y-3 max-h-[850px] overflow-y-auto custom-scrollbar pr-2">
                  {appointments.map((booking) => {
                    ...
                    let rowClass =
                      "bg-surface-container/20 border border-outline-variant/20 px-6 py-5 rounded-lg hover:border-primary group transition-all cursor-pointer";
                    let initialsClass =
                    ...
                    if (booking.uiStatus === "Chưa tới") {
                      rowClass =
                        "bg-surface-container/40 border border-outline-variant/50 px-6 py-5 rounded-lg hover:border-gold-dim transition-all cursor-pointer";
                    ...
                    const isFuture =
                      new Date(booking.rawDate).setHours(0, 0, 0, 0) >
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          router.push(
                            `/staff/bookings/detail?id=${booking._id}`,
                          )
                        }
                      >
                        <div className="col-span-3 flex items-center gap-3">
                          <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${initialsClass}`}
                          >
                            {initials}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="font-headline-sm text-sm font-bold text-on-surface truncate">
                              {booking.customerName}
                            </p>
                            <span
                            ...
                            return (
                              <button
                                onClick={(e) => {
                                  ...
                                  if (isFuture) {
                                    toast(
                                      "Lịch chưa tới, không thể thay đổi trạng thái",
                                    );
                                    return;
                                  }
                                  if (
                                    booking.rawStatus === "completed" ||
                                    booking.rawStatus === "no_show" ||
                                    booking.rawStatus === "cancelled" ||
                                    booking.rawStatus === "rejected"
                                  ) {
                                    toast(
                                      "Lịch đã đóng, không thể thay đổi trạng thái",
                                    );
                                    return;
                                  }
                                  setCheckInModal({
                                ...
                                className={`px-3 py-1 text-[10px] font-bold rounded uppercase tracking-wider flex items-center gap-1 transition-all active:scale-95 ${
                                  isFuture
                                    ? "bg-surface-variant text-on-surface-variant border border-outline-variant opacity-60 cursor-not-allowed"
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="glass-card p-4 rounded-xl flex items-center justify-between border-b-2 border-b-outline-variant">
            <div>
              <p className="text-[10px] font-label-md text-outline uppercase">
                Tổng lịch hẹn
              </p>
            ...
            <span className="material-symbols-outlined text-3xl text-outline/20">
              calendar_month
            </span>
          </div>
          ...
          <div className="glass-card p-4 rounded-xl flex items-center justify-between border-b-2 border-b-secondary">
            <div>
              ...
              <h3 className="text-xl font-bold text-secondary mt-1">
                {stats.serving < 10 ? `0${stats.serving}` : stats.serving}
              </h3>
            </div>
            <span className="material-symbols-outlined text-3xl text-secondary/20">
              content_cut
            </span>
          </div>
            ...
            {qrCodeData ? (
              <div className="p-6 text-center space-y-4">
                ...
                <p className="text-sm text-on-surface-variant">
                  Khách hàng quét mã này bằng ứng dụng ngân hàng
                </p>
              ...
              <div className="p-6 text-center space-y-4">
                ...
                <p className="text-sm text-on-surface-variant">
                  Vui lòng thu tiền trước khi hoàn thành lịch hẹn.
                </p>
                ...
                <div className="bg-surface-container-low border border-outline-variant/30 rounded-xl p-4 text-left space-y-2 mt-4">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-bold text-on-surface">
                      {(checkInModal.booking?.totalPrice || 0).toLocaleString(
                        "vi-VN",
                      )}
                      đ
                    </span>
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-bold text-primary">
                      {(checkInModal.booking?.amountPaid || 0).toLocaleString(
                        "vi-VN",
                      )}
                      đ
                    </span>
                  ...
                  <div className="pt-2 border-t border-outline-variant/30 flex justify-between">
                    ...
                    <span className="font-bold text-error text-lg">
                      {Math.max(
                        0,
                        (checkInModal.booking?.totalPrice || 0) -
                          (checkInModal.booking?.amountPaid || 0),
                ...
                <div className="flex flex-col gap-3 pt-4">
                  ...
                  <button
                    onClick={() =>
                      setCheckInModal((prev) => ({ ...prev, isPayment: false }))
                    }
              ...
              <div className="p-6 text-center space-y-4">
                ...
                <p className="text-sm text-on-surface-variant">
                  Chọn trạng thái tiếp theo của lịch hẹn
                </p>

                <div className="flex flex-col gap-3 pt-4">
                  {checkInModal.booking?.rawStatus === "completed" ||
                  checkInModal.booking?.rawStatus === "no_show" ||
                  checkInModal.booking?.rawStatus === "cancelled" ||
                  checkInModal.booking?.rawStatus === "rejected" ? (
                    <div className="p-4 bg-surface-container-low rounded-xl">
                      <p className="font-bold text-on-surface">
                        Lịch hẹn đã đóng, không thể thay đổi.
                      </p>
                      <button
                        onClick={() => {
                          setCheckInModal({
      ...
      {statusConfirmModal.isOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-surface-obsidian/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-surface-container-high rounded-2xl shadow-2xl w-full max-w-xs overflow-hidden animate-in zoom-in-95 duration-200 border border-outline-variant">
            <div className="p-6 text-center space-y-4">
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto ${statusConfirmModal.bg}`}
              >
                <span
                  className={`material-symbols-outlined text-3xl ${statusConfirmModal.color}`}
                >
                  {statusConfirmModal.icon}
                </span>
              </div>
              <h3
                className={`font-headline-sm text-xl font-bold ${statusConfirmModal.color}`}
              >
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  className={`flex-1 py-3 font-bold rounded-xl transition-all shadow-md text-white ${
                    statusConfirmModal.status === "cancelled" ||
                    statusConfirmModal.status === "no_show"
