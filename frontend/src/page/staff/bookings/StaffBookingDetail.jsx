    ...
    return (
      <div className="w-full min-h-[calc(100vh-80px)] flex justify-center items-center">
        <div className="flex flex-col items-center gap-4">
          <span className="material-symbols-outlined animate-spin text-4xl text-primary drop-shadow-[0_0_15px_rgba(212,175,55,0.5)]">
            autorenew
          </span>
          <span className="font-label-md text-on-surface-variant uppercase tracking-widest text-sm animate-pulse">
            Đang tải dữ liệu...
          </span>
        </div>
      </div>
      ...
      <div className="w-full min-h-[calc(100vh-80px)] flex justify-center items-center">
        <div className="glass-panel p-8 text-center max-w-md w-full border border-outline-variant/30 rounded-2xl flex flex-col items-center gap-4">
          ...
          <p className="font-body-md text-on-surface-variant mb-4">
            Lịch hẹn này có thể đã bị xóa hoặc không tồn tại.
          </p>
          <button
            onClick={() => router.push("/staff/bookings")}
            className="px-6 py-2.5 rounded-lg bg-surface-container hover:bg-surface-variant text-primary font-label-md tracking-wider uppercase transition-all duration-300 active:scale-95 border border-outline-variant/50"
          >
            Quay lại danh sách
          </button>
        </div>
    ...
    switch (status) {
      case "pending":
        return {
          text: "Chưa tới",
          icon: "pending",
          color: "bg-surface-bright/50 text-gold-dim border-gold-dim/30",
        };
      case "completed":
        return {
          text: "Hoàn thành",
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-6 pt-6">
        <div>
          ...
          <div className="flex items-center gap-3 mb-3">
            <span
              className={`px-3 py-1.5 rounded-full border ${statusInfo.color} font-label-md text-xs uppercase tracking-widest flex items-center gap-1.5 backdrop-blur-sm shadow-sm`}
            >
        ...
        <div className="glass-panel relative overflow-hidden bg-surface-container-low/40 border border-outline-gold/30 rounded-2xl p-4 md:px-6 md:py-4 flex items-center gap-5 min-w-[220px] shadow-[0_8px_32px_rgba(0,0,0,0.2)]">
          ...
          <div className="flex flex-col">
            <span className="font-label-md text-[10px] text-on-surface-variant uppercase tracking-widest mb-1">
              Giờ hẹn
            </span>
            <span className="font-display-md text-2xl font-bold text-primary tracking-tighter drop-shadow-sm">
              {booking.time ||
                (booking.rawDate
                  ? new Date(booking.rawDate).toLocaleTimeString("vi-VN", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })
                  : "N/A")}
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="glass-panel relative overflow-hidden bg-surface-container-low/60 border border-outline-variant/40 hover:border-outline-gold/50 rounded-2xl p-6 transition-all duration-300 hover:shadow-[0_8px_32px_rgba(212,175,55,0.05)] group">
            ...
            <div className="flex items-center gap-4 relative z-10">
              ...
              <div>
                <h3 className="font-headline-sm text-on-surface text-lg mb-1">
                ...
                {booking.services?.map((service, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <h4 className="font-headline-sm text-on-surface text-base md:text-lg mb-1">
                        {service.name}
                      </h4>
                    ...
                    {booking.products.map((p, index) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4">
                          <span className="font-display-md text-lg md:text-xl font-bold text-on-surface tracking-tight whitespace-nowrap">
                            {(
                              (p.priceAtPurchase || 0) * p.quantity
              ...
              <div className="border-t border-outline-variant/50 pt-6 mt-auto">
                ...
                <div className="flex justify-between items-end bg-surface-container p-6 rounded-xl border border-outline-gold/20 shadow-inner">
                  ...
                  <span className="font-display-lg text-3xl md:text-4xl font-extrabold text-primary tracking-tighter drop-shadow-md">
                    ...
                    <span className="text-xl text-primary/70 font-normal">
                      đ
                    </span>
                  </span>
                </div>
        ...
        {booking.status === "confirmed" && (
          <>
            <button
              ...
              className="px-6 py-3 bg-surface-danger text-error font-bold text-xs uppercase tracking-wider border border-error/50 rounded-lg hover:bg-error/10 transition-all active:scale-95"
            >
              Hủy Lịch
            </button>
            <button
              ...
              <button
                onClick={() => {
                  setStatusConfirmModal({
                    ...
                    message:
                      "Đơn hàng này đã thanh toán đủ. Xác nhận đóng đơn?",
                    icon: "check_circle",
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="glass-panel w-full max-w-sm rounded-2xl border border-outline-variant/50 shadow-2xl bg-surface-container overflow-hidden">
            <div className="p-6 text-center">
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${statusConfirmModal.bg}`}
              >
                <span
