    ...
    const fetchData = async () => {
      try {
        const rosterRes = await axios.get(
          "http://localhost:5000/api/rosters/current/active",
          {
        ...
        if (rosterRes.data.success && rosterRes.data.roster) {
          ...
          const regRes = await axios.get(
            `http://localhost:5000/api/rosters/${currentRoster._id}/my-registration`,
            {
              withCredentials: true,
            },
          );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const registeredShifts = days.map((day) => {
        const shiftsForDay = [];
        if (selectedShifts[`${day.dateStr}-morning`])
          shiftsForDay.push("morning");
        if (selectedShifts[`${day.dateStr}-afternoon`])
          shiftsForDay.push("afternoon");
        return {
          date: day.dateStr,
          shifts: shiftsForDay,
        };
      ...
      const res = await axios.post(
        `http://localhost:5000/api/rosters/${roster._id}/register`,
        payload,
        {
          withCredentials: true,
        },
      ...
      alert(
        error.response?.data?.message || "Đã xảy ra lỗi khi đăng ký lịch làm.",
      );
    } finally {
      ...
      <main className="pt-8 pb-section-padding px-4 md:px-margin-desktop max-w-[1400px] mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-on-surface-variant max-w-2xl mb-8">
            Xem lịch làm việc cá nhân hoặc cập nhật khung thời gian làm việc hàng tuần của bạn. 
            Lưu ý lịch đăng ký sẽ được mở vào thứ 5 hàng tuần và chốt vào thứ 7.
          </p>
          ...
          <div className="flex gap-8 border-b border-outline-variant/30">
            <button
              ...
              className={`pb-4 font-headline-sm tracking-wide transition-all ${
                activeTab === "schedule" 
        ...
        {activeTab === "schedule" ? (
          ...
          <div className="bg-surface-container border border-outline-variant p-10 rounded-xl text-center shadow-sm">
            ...
            <h3 className="font-headline-md text-headline-md text-on-surface mb-2">
              Không có lịch đăng ký nào đang mở
            </h3>
            <p className="text-on-surface-variant">
              Chưa mở lượt đăng ký lịch làm cho tuần mới. Quay lại vào thứ 5 để
              đăng ký.
            </p>
          </div>
        ) : (
          <>
            {/* Status Banner */}
            <div
              className={`bg-surface-container border-l-4 ${registration ? "border-green-500" : "border-primary"} p-6 rounded-r-xl mb-10 flex items-start gap-4 shadow-sm`}
            >
              <span
                className={`material-symbols-outlined ${registration ? "text-green-500" : "text-primary"} text-3xl`}
              >
              ...
              <div>
                <h3 className="font-headline-sm text-headline-sm text-on-surface mb-1">
                  ...
                  >
                    {registration?.status || "Chưa đăng ký"}
                  </span>
                </h3>
                <p className="text-on-surface-variant">
                  Hiện tại hệ thống đang mở nhận lịch cho tuần:{" "}
                  <span className="text-primary font-bold">
                    {formatDateStr(roster.weekStartDate)} -{" "}
                    {formatDateStr(roster.weekEndDate)}
                  </span>
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8">
                ...
                <div className="bg-surface-container-low border border-outline-variant rounded-xl p-6 md:p-8 overflow-x-auto custom-scrollbar">
                  ...
                  <div className="grid grid-cols-7 gap-2 md:gap-4 min-w-[600px]">
                    {days.map((day) => (
                      <div
                        key={day.dateStr}
                        className={`flex flex-col items-center ${day.isClosed ? "opacity-50 pointer-events-none" : ""}`}
                      >
                        <div className="mb-6 text-center">
                          <p className="text-label-md text-outline uppercase mb-1">
                            {day.dayOfWeek}
                          </p>
                        ...
                        <div className="flex flex-col gap-3 w-full">
                          {day.isClosed ? (
                            ...
                            SHIFTS.map((shift) => {
                              const isSelected =
                                selectedShifts[`${day.dateStr}-${shift.id}`];
                              return (
                                <button
                                  key={shift.id}
                                  onClick={() =>
                                    toggleShift(day.dateStr, shift.id)
                                  }
              ...
              <div className="h-fit sticky top-28">
                <div className="bg-surface-container-low border border-outline-variant rounded-xl p-6 md:p-8">
                  ...
                  <div>
                    <p className="font-label-md text-label-md text-outline uppercase tracking-widest mb-6">
                      Quy định
                    </p>
