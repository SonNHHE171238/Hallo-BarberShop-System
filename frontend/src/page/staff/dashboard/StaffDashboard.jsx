...
export default function StaffDashboard() {
  ...
  useEffect(() => {
    if (qrCodeData && checkInModal.isOpen && checkInModal.booking) {
      const updatedBooking = todayBookings.find(
    ...
    try {
      ...
      if (res) {
        if (status === "confirmed" && previousStatus === "pending") {
          setMetrics((prev) => ({
            ...prev,
            waitingCustomers: prev.waitingCustomers + 1,
          }));
        }
                      ...
                      {todayBookings.length === 0 ? (
                        ...
                        todayBookings.map((booking) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-5 text-sm text-on-surface-variant italic">
                              {booking.serviceName}
                            </td>
          ...
          <div className="flex flex-col min-h-0">
            ...
            <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar flex-1 min-h-0">
              {barbersStatus.length === 0 ? (
                <div className="glass-card p-6 text-center text-on-surface-variant text-sm rounded-xl">
                  Không có thợ nào hoạt động
                </div>
              ) : (
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        <div className="relative">
                          <div
                            className={`w-12 h-12 rounded-full bg-surface-container-high border border-outline-variant overflow-hidden transition-all ${imgClass}`}
                          >
                            <img
                          ...
                          <span
                            className={`absolute bottom-0 right-0 w-3 h-3 border-2 border-surface rounded-full ${ringClass}`}
                          ></span>
                        </div>
                        <div>
                      ...
                      <span
                        className={`text-[10px] font-bold uppercase tracking-widest border px-2 py-1 rounded ${statusClass}`}
                      >
                        {barber.status}
                      </span>
                    </div>
          ...
          <div className="bg-surface-container-high rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-outline-variant">
            {qrCodeData ? (
              ...
              <div className="p-6 text-center space-y-4">
                ...
                <h3 className="font-headline-sm text-xl text-on-surface font-bold">
                  Xác nhận trạng thái khách hàng
                </h3>
                <p className="text-sm text-on-surface-variant">
                  Khách đã tới cửa hàng chưa?
                </p>

                <div className="flex flex-col gap-3 pt-4">
                  {checkInModal.booking?.status === "completed" ||
                  checkInModal.booking?.status === "no_show" ||
                  checkInModal.booking?.status === "cancelled" ||
                  checkInModal.booking?.status === "rejected" ? (
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className={`flex-1 px-4 py-2 text-white rounded font-label-md transition-colors shadow-sm ${
                    statusConfirmModal.status === "confirmed"
