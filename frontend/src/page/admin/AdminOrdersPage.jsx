...
export default function AdminOrdersPage() {
  ...
  const mapOrderStatus = (status) => {
    const map = {
      pending: { label: "Đơn mới", color: "bg-primary/20 text-primary border-primary/30" },
      processing: { label: "Đang chuẩn bị", color: "bg-surface-variant text-on-surface-variant border-outline-variant" },
      shipped: { label: "Đang giao", color: "bg-secondary-fixed-dim/20 text-secondary border-secondary-fixed-dim/30" },
      completed: { label: "Hoàn thành", color: "bg-green-500/20 text-green-400 border-green-500/30" },
      cancelled: { label: "Đã hủy", color: "bg-error/20 text-error border-error/30" }
    };
    return map[status] || { label: status, color: "bg-surface-variant text-on-surface-variant border-outline-variant" };
  };
        ...
        <nav className="flex gap-8 min-w-max">
          {TABS.map(tab => (
            <button 
              ...
              className={`pb-4 whitespace-nowrap text-label-md uppercase transition-colors ${
                filterTab === tab 
            ...
            <tbody className="divide-y divide-outline-variant/30">
              {loading ? (
                ...
                filteredOrders.map(order => {
                  const orderStat = mapOrderStatus(order.status);
                    ...
                    <tr key={order._id} className="hover:bg-surface-variant/30 transition-all group">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[11px] text-on-surface-variant">{formatDate(order.createdAt)}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
